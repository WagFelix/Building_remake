...
func _on_mute_button_down() -> void:
	var wVolume := db_to_linear(AudioServer.get_bus_volume_db(AudioServer.get_bus_index("Musicas")))
	...
	functions.MUSICV=nVolume
	functions.save_settings(false)	
	MusicController._volume(nVolume, "Musicas")
...
#		functions.save_config=true
		functions.MUSICV=newvolume
		if newvolume==0:
			$mute/Cancel.visible=true
		else:
			$mute/Cancel.visible=false
		MusicController._volume(newvolume, "Musicas")
		functions.save_settings(false)
		
		print(newvolume)
#		GlobalWorldEnvironment._playMusic(0)

func setValue(slider):
	slider.value = int(ratioInBody(slider) * slider.max_value)
